...
root for logged precompile claims; `DeferredProof` explains why that root should be accepted. The
processor default registry is empty; public VM verifier facades install the standard
`miden-precompiles` registry for ordinary verification.
