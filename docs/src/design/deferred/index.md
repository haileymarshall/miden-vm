...
- `ExecutionProof` carries a `DeferredProof` envelope: empty proofs resolve to `TRUE_DIGEST`, wire
  proofs carry partial `DeferredStateWire`, and STARK-backed proofs carry a precompile VM STARK proof
  for the exact deferred root;
