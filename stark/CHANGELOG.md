...
- [BREAKING] Replaced the `(AIR, AirWitness, AuxBuilder)` proving model with a `MultiAir` trait that owns its AIRs and builds their aux traces (`build_aux_traces`), plus validated `Statement` / `ProverStatement` structs. `prove` / `verify` (renamed from `prove_multi` / `verify_multi`; `prove_single` / `verify_single` removed) take `&ProverStatement` / `&Statement` and a separate `config`. Heterogeneous AIRs are expressed via caller-defined enum wrappers (the `LiftedBenchAir` pattern in `miden-bench`). ([#992](https://github.com/0xMiden/crypto/pull/992))
...
- [BREAKING] AIR ordering is no longer public: `InstanceShape` / `InstanceShapes` are removed and `StarkProof` carries a single `log_trace_heights: Vec<u8>` (instance order, no public accessor; the prior `air_indices` field is gone). The wire-format ordering is derived deterministically from the heights (stable sort on `(log_h, instance_idx)`); read the heights and derived order via `StarkTranscript::log_trace_heights()` / `StarkTranscript::air_order()`. ([#992](https://github.com/0xMiden/crypto/pull/992))
