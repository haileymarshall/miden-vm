...
fuzz-all: fuzz-seeds ## Run all fuzz targets (in sequence)
	...
	cargo +nightly fuzz run precompile_request_deserialize --release --fuzz-dir tools/miden-core-fuzz -- -max_total_time=300 || FAILED=1; \
	cargo +nightly fuzz run precompile_request_serde_deserialize --release --fuzz-dir tools/miden-core-fuzz -- -max_total_time=300 || FAILED=1; \
